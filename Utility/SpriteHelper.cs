...
namespace AssetStudio
{
    public static class SpriteHelper
    {
        ...
        public static Image<Bgra32> CutImage(Sprite m_Sprite, Texture2D m_Texture2D, Rectf textureRect, Vector2 textureRectOffset, float downscaleMultiplier, SpriteSettings settingsRaw)
        {
            ...
            if (originalImage != null)
            {
                using (originalImage)
                {
                    CutImage(m_Sprite, originalImage, textureRect, textureRectOffset, downscaleMultiplier, settingsRaw);
                }
            }
        ...
        public static Image<Bgra32> CutImage(Sprite m_Sprite, Image<Bgra32> originalImage, Rectf textureRect, Vector2 textureRectOffset, float downscaleMultiplier, SpriteSettings settingsRaw)
        {

            if (downscaleMultiplier > 0f && downscaleMultiplier != 1f)
            {
                var width = (int)(originalImage.Width / downscaleMultiplier);
                var height = (int)(originalImage.Height / downscaleMultiplier);
                originalImage = originalImage.Clone(x => x.Resize(width, height));
            }
